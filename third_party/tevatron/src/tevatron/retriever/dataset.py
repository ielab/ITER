...
class EncodeDataset(Dataset):
    ...
    def __init__(self, data_args: DataArguments):
        ...
        # Sort this shard by length so batches are near-uniform -> minimal padding.
        # Done after sharding, so only ~1/N rows are materialised. select() is an
        # index map (no table rewrite). Order is irrelevant to the output: each row
        # carries its docid into the (reps, lookup) pickle.
        if getattr(self.data_args, "encode_sort_by_length", False):
            col = "query_text" if self.data_args.encode_is_query else "text"
            if col not in self.encode_data.column_names:
                col = "query" if "query" in self.encode_data.column_names else None
            if col is not None:
                lengths = [len(t or "") for t in self.encode_data[col]]
                order = sorted(range(len(lengths)), key=lengths.__getitem__)
                self.encode_data = self.encode_data.select(order)
