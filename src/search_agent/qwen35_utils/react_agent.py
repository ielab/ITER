...
# --tool-call-parser / --reasoning-parser (see job_qwen35_smoke.sh).
#
# Why: this project's own openai_client.py native-FC path (tools=[] API field +
# vLLM's server-side qwen3_xml tool-call-parser + qwen3 reasoning-parser) hit
# repeated issues on this model family (ninja JIT-compile crash, TP2 NCCL hangs,
# forced-round tool_calls leaking past an empty `tools` request, continue_final_message
# returning zero generated tokens, and — the one that ended the native-FC attempt --
# ~30-40% of trajectories silently stopping mid-run with empty content despite
# tool_calls being available). Yuqi Zhou's own XIR A-leaderboard eval scripts for
# this exact model (github.com/Yuqi-Zhou/LRAT/tree/main/xir_a_leaderboard_eval)
# launch vLLM with NEITHER flag, confirming raw-text tool-call parsing (this
# project's own Tongyi/WebExplorer convention) is the validated integration path
# for this model family, not vLLM's native structured tool-calling.
...
class MultiTurnReactAgent(FnCallAgent):
    ...
    def call_server(self, msgs, planning_port, max_tries=10, prefill=None):
        ...
        string via vLLM's `continue_final_message` (assistant-prefill).
        """
        ...
        for attempt in range(max_tries):
            try:
                chat_response = client.chat.completions.create(
                    ...
                    seed=2026,
                    presence_penalty=self.llm_generate_cfg.get('presence_penalty', 1.5),
        ...
        while num_llm_calls_available > 0:
            ...
            if finish_reason == "length" and num_llm_calls_available > 0:
                ...
                truncated_msg = (
                    ...
                    "Now, skip all reasoning and directly provide the <tool_call> or final answer."
                )
