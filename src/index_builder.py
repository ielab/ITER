...
class Index_Builder:
    ...
    def build_bm25_index(self):
        ...
        # Step 1: Convert the corpus to Pyserini's JSON collection schema.
        with open(self.corpus_path, "r", encoding="utf-8") as source_file, \
                open(temp_file_path, "w", encoding="utf-8") as target_file:
        ...
        # Use all allocated cores (respects the SLURM cpuset); override with BM25_THREADS.
        try:
        ...
        try:
            threads = int(os.getenv("BM25_THREADS", default_threads))
        except ValueError:
            threads = default_threads

        logger.info("Starting BM25 index build with %d threads", threads)
        pyserini_args = [
        ...
        # Step 2: Build the Lucene index and verify every corpus row was indexed.
        subprocess.run(["python", "-m", "pyserini.index.lucene"] + pyserini_args, check=True)

        from pyserini.index.lucene import LuceneIndexReader
        indexed_documents = LuceneIndexReader(self.save_dir).stats()["documents"]
        if indexed_documents != len(self.corpus):
            raise RuntimeError(
                f"BM25 index contains {indexed_documents} documents; expected {len(self.corpus)}"
            )
