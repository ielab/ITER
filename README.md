...
<p align="center">
  <em>ITER builds a history-conditioned query from the main question, pre-search
...
- **A history-conditioned query.** The retriever input carries the main
  question, the agent's pre-search reasoning, the current sub-query, and the
  sub-queries already tried — a compact record of the current search intent and
  which directions have been explored, without the text of what was read.
...
ITER constructs its training data directly from agent trajectories, using
interaction signals already produced during trajectory collection. It requires
neither gold document labels nor oracle-augmented reranking.

Across six agent backbones from three model families, the 0.6B ITER outperforms
LRAT in all 12 backbone × benchmark comparisons, with average relative
improvements of **6.9%** on InfoSeek-Eval and **15.4%** on BrowseComp-Plus. At
the matched 4B scale, ITER outperforms AgentIR on InfoSeek-Eval for five of six
backbones and achieves a higher visit-to-search recall ratio on BrowseComp-Plus
across all six backbones. The paper reports the full tables.
...
| **`i7`** | i2 + pre-search reasoning — **the default** |
...
| ITER 0.6B *(default, `i7`)* | [`ielabgroup/ITER-Qwen3-Embedding-0.6B`](https://huggingface.co/ielabgroup/ITER-Qwen3-Embedding-0.6B) |
| ITER 4B *(default, `i7`)* | [`ielabgroup/ITER-Qwen3-Embedding-4B`](https://huggingface.co/ielabgroup/ITER-Qwen3-Embedding-4B) |

The two released checkpoints use the default query representation, `i7`. The
...
`python run_encode.py --setting i7 ...` prints the exact query and instruction
