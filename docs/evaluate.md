...
```bash
python run_eval.py \
  --setting i2 --backbone tongyi --benchmark bcp \
  --index /path/to/index/i2_bcp/index-000.pkl \
  --port <PORT>
```
...
| `i2` | the paper's default |
| `i2-4b` | the 4B scaling run |
...
python analysis/llm_judge_all.py \
  --dirs iter_i2=/path/to/runs/bcp_tongyi_i2 lrat=/path/to/runs/bcp_tongyi_lrat \
